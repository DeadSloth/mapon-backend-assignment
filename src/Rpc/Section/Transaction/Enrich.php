...
class Enrich extends Base
{
    ...
    public function __construct(stdClass $params)
    {
        ...
        $this->transactionId = $this->requireParam('id', 'int');
    }
    ...
    public function process(): array
    {
        ...
        $transaction = $enrichmentService->processSingle(
            $repository->getById($this->transactionId)
        );

        return [
            'items' => [TransactionDTO::fromModel($transaction)->toArray()],
            'success' => $transaction->isEnriched(),
            'message' => $enrichmentService->getLatestMessage(),
        ];
    }
}
